...
const addCommonEventData = (event, deviceIdCookieValue, sessionIdCookieValue) => {
  ...
  if(deviceIdCookieValue){
    event.client_id = deviceIdCookieValue;
  }
  if(sessionIdCookieValue){
    event.session_id = sessionIdCookieValue;
  }
  return event;
};
...
let setOrUpdateCookie = (cookieName, domain, cookiePath, cookieSecure, cookieHttpOnly, cookieSameSite, duration, value, responseKey) => { 
    ...
    //set response
    responseData[responseKey] = cookieValue;
    return cookieValue;
};
